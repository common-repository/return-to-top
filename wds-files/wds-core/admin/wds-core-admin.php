	...
	*/
	...
	function add_page() {
		...
		if ( ! is_multisite() || (is_multisite() && !WDS_SITEWIDE)) {
			$wds_page_hook = add_submenu_page( 'options-general.php', __( 'CHAR DEV SEO Wizard' , 'wds'), __( 'CHAR DEV SEO', 'wds' ), $this->capability, 'wds_wizard', array( &$this, 'options_page' ) );
		} else if (is_multisite() && version_compare( $wp_version , '3.0.9', '>' ) ) {
			$wds_page_hook = add_submenu_page( 'settings.php', __( 'CHAR DEV SEO Wizard' , 'wds'), __( 'CHAR DEV SEO', 'wds' ), $this->capability, 'wds_wizard', array( &$this, 'options_page' ) );
		} else if (is_multisite()) {
			$wds_page_hook = add_submenu_page( 'ms-admin.php', __( 'CHAR DEV SEO Wizard' , 'wds'), __( 'CHAR DEV SEO', 'wds' ), $this->capability, 'wds_wizard', array( &$this, 'options_page' ) );
		}
...
class WDS_Core_Admin_Tab extends WDS_Core_Admin {
	...
	*/
	...
	function options_page() {
		...
		echo "<div class='wrap'>$msg
			<div class='icon32' id='icon-options-general'><br></div>
			<h2>" . __( 'CHAR DEV SEO Wizard', 'wds' ) . "</h2>
